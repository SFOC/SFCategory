...
-(void)setImage:(UIImage *)image
{
    ...
    _myImageView.frame = CGRectMake(0, 0, self.frame.size.width*1.15, self.frame.size.height);
...
-(void)startAnimate
{
    ...
    [[YGGravity sharedGravity]startDeviceMotionUpdatesBlock:^(float x, float y, float z) {
        ...
        [UIView animateKeyframesWithDuration:0.3 delay:0 options:UIViewKeyframeAnimationOptionCalculationModeDiscrete animations:^{
            ...
            {
                float invertedYRotationRate = y * -2.0;
...
//                float invertedXRotationRate = x * -9.0;
//                float interpretedYOffset = _myImageView.frame.origin.x + invertedXRotationRate * (_myImageView.frame.size.width/[UIScreen mainScreen].bounds.size.width) * scrollSpeed + _myImageView.frame.size.width/2;
                
                _myImageView.center = CGPointMake(interpretedXOffset, _myImageView.center.y);
//                _myImageView.center = CGPointMake(interpretedXOffset, interpretedYOffset);
            }
