...
//配置密码
#define HIDE_FUNCTION_OPEN_KEY @"3344"
...
- (void)setPwdStr:(NSMutableString *)pwdStr {
  objc_setAssociatedObject(self, &PWDSTR, pwdStr,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (HideFunctionGridType)gridType {
  return (HideFunctionGridType)[objc_getAssociatedObject(self, &GridTypeStr)
                                    integerValue];
}

- (void)setGridType:(HideFunctionGridType)gridType {
  objc_setAssociatedObject(self, &GridTypeStr,
                           [NSNumber numberWithInt:gridType],
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)addHideFunction:(void (^)())successCallBack {
  objc_setAssociatedObject(self, &SuccessCallback, successCallBack,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)tapPressed:(UITapGestureRecognizer *)recognizer {
  ...
  NSLog(@"self.pwdStr)== %@", self.pwdStr);
}

- (void)handleSwipeFrom:(UISwipeGestureRecognizer *)recognizer {
  if (recognizer.direction == UISwipeGestureRecognizerDirectionUp &&
      [self.pwdStr isEqualToString:HIDE_FUNCTION_OPEN_KEY]) {
...
//    if (CGRectContainsPoint(CGRectMake(0, 0, 320, 160), currentPoint) ) {
//        return NO;
//    }
//    return YES;
//}
